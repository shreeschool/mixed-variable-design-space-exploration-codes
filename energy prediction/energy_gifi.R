setwd("D:/IIT Madras/Gifi methods/energy prediction problem")
library(readxl)
data=read_excel('enrgy_for_som.xlsx')
...
M_mixed1<-data.frame(x1,x2,x3,x4,x5,x6,x7,x8,x9)
...
item_knots<-Gifi::knotsGifi(
  x=M_mixed1[,c('x1','x2','x3','x4')],
  type='Q',n=4
) ##item knots(data)
species_knots<- Gifi::knotsGifi(
  x=M_mixed1[,c('x5','x6','x7','x8','x9')],
  type='Q',n=4
) ##method knots(data)
...
V_ordinal<- c(
  rep(TRUE,5),rep(TRUE,1),rep(TRUE,3)
...
library("writexl")
write_xlsx(data.frame(princals_mixed_iris[["scoremat"]]),"D:/IIT Madras/Gifi methods/energy prediction problem/energy_som_conversion.xlsx")
