...
%%
Tb = readtable('crank_conversion_exp2.xlsx');
d = table2array(Tb);
data = d(:,1:6);
F(:,1) = data(:,6);
% F(:,2) = data(:,4);
...
sData = som_data_struct(test,'my-data','comp_names',{'X1','X2','X3','X4','Y'});
sData = som_normalize(sData,'range');

sData1 = som_data_struct(test1,'my-data','comp_names',{'X1','X2','X3','X4','Y'});
sData1 = som_normalize(sData1,'range');

% sData2 = som_data_struct(test2,'my-data','comp_names',{'x1','x2','F2'});
...
[sMap,A,V]= modifiedsom_lininitcrank_exp2(sData,'lattice','hexa','msize',[5,5]);
...
% sMap_umatrix.codebook(:,5) = sMap2.codebook(:,5);
...
% sMap_umatrix.codebook(:,9) = sMap7.codebook(:,3);
% sMap_umatrix.codebook(:,10) = sMap5.codebook(:,3);

hx(find(sMap_umatrix.codebook(:,end)<3.1))=1;
Tb2 = readtable(['crank_codebook_exp2.xlsx']);
d2 = table2array(Tb2);
data2 = d2(:,1:6);
sMap_umatrix2.codebook(:,1:6)=data2;
[mqe,tge] = som_quality(sMap_umatrix,d(:,1:6));
[mqe2,tge2] = som_quality(sMap_umatrix2,data2);
...
hx(find(sMap_umatrix2.codebook(:,end)<0.050))=1;
hx1(find(sMap_umatrix2.codebook(:,end)>0.00))=1;
% Visualization of SOM results( U Matrix and Component Planes )
figure(2)
som_show(sMap_F);
figure(3)
som_show(sMap_umatrix);
som_show_add('hit',som_hits(sMap_umatrix,data),'Markersize',0.5,'markercolor','k','Subplot',2:7)
figure(4)
som_show(sMap_umatrix2);
% som_show_add('hit',som_hits(sMap_umatrix,data),'Markersize',0.5,'markercolor','k','Subplot',2:7)
% som_show_add('hit',hx,'Markersize',0.2,'Markercolor','k','Edgecolor','k','Subplot',2:7)
% som_show_add('hit',hx,'Markersize',0.5,'Markercolor','k','Edgecolor','k','Subplot',2:7)
% som_show_add('hit',hx,'Markersize',0.2,'Markercolor','k','Edgecolor','k','Subplot',2:7)
% % som_show_add('hit',hx,'Markersize',1,'Markercolor','none','Edgecolor','r','Subplot',1:3)
som_show_add('hit',hx1,'Markersize',1,'Markercolor','none','Edgecolor','k','Subplot',2:7)
% som_show_add('hit',t1,'SubPlot',[3,4])
% som_show_add('hit',t2,'SubPlot',[3,5])
% som_show_add('hit',t3,'SubPlot',[2,3])
% som_show_add('hit',t4,'SubPlot',[2])

figure(5)
som_grid(sMap_umatrix,'Coord',sMap_umatrix.codebook(:,[1,3,5]))
hold on, scatter3(d(:,1),d(:,3),d(:,5),'+r')
title('Trained map')
