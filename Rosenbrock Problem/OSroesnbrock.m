...
%%
Tb = readtable('conversion_ros_exp2.xlsx');  
d = table2array(Tb);
data = d(:,1:3); % Select the number of columns
F(:,1) = data(:,3); % Select all the columns that you want to treat as output
% F(:,2) = data(:,3); %
% F(:,3) = data(:,5); %
% F(:,4) = data(:,6);
% F(:,5) = data(:,7);
...
[sMap,A,V]= modifiedsom_lininitrosexp2(sData,'lattice','hexa','msize',[5,5]);
...
Tb2 = readtable('codebook_ros_exp2.xlsx');  
d2 = table2array(Tb2);
data2 = d2(:,1:3);
...
hx(find(sMap_umatrix2.codebook(:,end)<3500))=1;
hx1(find(sMap_umatrix2.codebook(:,end)>-9000))=1;
...
% som_show_add('hit',hx,'Markersize',0.2,'Markercolor','k','Edgecolor','k','Subplot',2:4)
% som_show_add('hit',hx,'Markersize',1,'Markercolor','none','Edgecolor','r','Subplot',1:3)

figure(5)
som_grid(sMap_umatrix,'Coord',sMap_umatrix.codebook(:,[1,2,3]))
hold on, scatter3(d(:,1),d(:,2),d(:,3),'+r')
title('Trained map')